"""
Model trainer that's simplified from previous versions.
Todo: make all training stuff organized, as I get lost everyday I come back to this
"""
import os
import pandas as pd
import numpy as np
...
def train_fitness_model(test_size=0.2, cv_folds=5):
    ...
    def calc_rtss(row):
        ...
        return alg.calculate_rtss(gap, row['Seconds'], THRESHOLD_PACE)
    
    df_running['rTSS'] = df_running.apply(calc_rtss, axis=1)
    
    print(f"Average rTSS: {df_running['rTSS'].mean():.1f}")

    # Optimize Banister params
    from scipy.optimize import minimize
    
    load_history = df_running['rTSS'].values
    actual_perf = df_running['Speed_mps'].values
    hr_data = df_running['Avg HR'].values
    ...
    X, y = [], []
    window = 7
    for i in range(window, len(df_running)):
        if actual_perf[i] < 1.0:
            continue
        
        win_slice = df_running.iloc[i-window : i]
        feat = [
            win_slice['rTSS'].mean(),
            win_slice['Total Ascent'].mean(),
            df_running['Avg HR'].iloc[i]
        ]
        X.append(feat)
